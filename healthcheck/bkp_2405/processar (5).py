...
def calcular_metricas_consultor(universo, df_visitas, df_ausencia_dia):
    ...
    for _, p in universo.iterrows():
        ...
        if len(aus_p):
            ...
            mask_3m = aus_p_dt >= (cutoff_3m - pd.offsets.MonthBegin(0))
            mask_1m = aus_p_dt >= (cutoff_1m - pd.offsets.MonthBegin(0))
            aus_3m_real = aus_p[mask_3m]['fr_total'].sum()
            aus_1m_real = aus_p[mask_1m]['fr_total'].sum()
        ...
        if INCLUIR_MES_PARCIAL:
            ...
            # para todo mundo. Mesma armadilha já tinha sido tratada para visitas (que
            # leem df_visitas direto). Aqui aplicamos o mesmo padrão: ler
            # df_ausencia_dia direto, filtrando apenas pelo ISID e pelo mês parcial.
            ...
            fator_pp = (dias_decorridos_parcial / dias_uteis_parcial_completo) if dias_uteis_parcial_completo > 0 else 0
            if len(aus_isid_total):
                ...
                aus_parcial_mes = aus_parcial_filtro['fr_total'].sum()
                # Prorratear: ausência registrada × (dias decorridos / dias úteis totais do mês)
                aus_parcial = aus_parcial_mes * fator_pp
                # Detalhamento por categoria (mesmo prorrateamento)
        ...
        rows.append({
            ...
            # Onda v3 — série de VISITAS/MÊS (mesma janela 3-6m do vd_6m_serie).
            # Usada pelo ICV no html.py: 1 − coef. variação das visitas mensais.
            # Métrica fiel à fórmula original (visitas, não vis/dia).
...
def calcular_meses_no_setor_v2(df_metr: pd.DataFrame,
                                historico: dict,
                                mes_ref: 'date' = None) -> pd.DataFrame:
    """
    ...
    print(f"    meses_no_setor v2: {n_pop}/{len(df_metr)} populados "
          f"({n_quebra} com quebra de equipe detectada)")
    return df_metr
...
def montar_consultores(df_metr):
    ...
    for _, c in df_metr.iterrows():
        ...
        # ====================================================================
        # NOVO (Onda v5): cobertura e painel ocioso POR JANELA
        # --------------------------------------------------------------------
        # Antes esses campos eram populados como None — ICO/IPO no html.py
        # ficavam exibindo "—" para todo mundo. Agora calculados aqui a partir
        # de mdms_visitados_<jan> ∩ mdms_painel, para cada uma das 4 janelas.
        #
        # Definições:
        #   - cobertura_mensal_pct_<jan> = % do painel coberto (média mensal)
        #     • mat (12 meses): mdms cumulativos ÷ 12 ÷ painel × 100
        #     • 3m: mdms cumulativos ÷ 3 ÷ painel × 100
        #     • 1m: mdms cumulativos ÷ painel × 100 (já é mensal)
        #     • parcial: mdms cumulativos ÷ (dias_decorridos/dias_uteis) ÷ painel × 100
        #   - medicos_sem_visita_60d_<jan> = painel − interseção(painel, visitados_jan)
        #     Renomeado conceitualmente para "médicos do painel não tocados na janela",
        #     pra ficar consistente com a janela escolhida.
        # ====================================================================
        painel_set = set(c.get('mdms_painel') or [])
        painel_n = len(painel_set)
        # Helper: calcula cobertura+ocioso para uma janela específica
        def _cob_oc_janela(mdms_visitados_jan, n_meses):
            if painel_n == 0 or not mdms_visitados_jan:
                return None, None
            visit_set = set(mdms_visitados_jan or [])
            intersec = len(visit_set & painel_set)
            ocioso = painel_n - intersec  # médicos do painel não tocados na janela
            if n_meses and n_meses > 0:
                cob_mensal = round(100.0 * intersec / n_meses / painel_n, 2)
            else:
                cob_mensal = round(100.0 * intersec / painel_n, 2)
            return cob_mensal, ocioso

        cob_mat, oc_mat = _cob_oc_janela(c.get('mdms_visitados_mat') or [], 12)
        cob_3m,  oc_3m  = _cob_oc_janela(c.get('mdms_visitados_3m')  or [], 3)
        cob_1m,  oc_1m  = _cob_oc_janela(c.get('mdms_visitados_1m')  or [], 1)
        # parcial: anualiza pelo proporcional do mês decorrido
        _dias_decorridos = int(c.get('parcial_dias_decorridos') or 0)
        _dias_uteis_mes  = float(c.get('uteis_mes') or 20)
        _fator_parcial = (_dias_decorridos / _dias_uteis_mes) if _dias_uteis_mes > 0 else 1.0
        cob_par, oc_par = _cob_oc_janela(c.get('mdms_visitados_parcial') or [],
                                          _fator_parcial if _fator_parcial > 0 else 1.0)
        ...
        d = {
            ...
            # ────────────────────────────────────────────────────────────────
            # NOVO (Onda v5): cobertura e painel ocioso por janela
            # Calculados acima a partir de mdms_visitados_<jan> ∩ mdms_painel.
            # Substituem os campos antigos (cobertura_mensal_pct, medicos_sem_visita_60d)
            # que ficavam None. Mantemos os antigos como ALIAS da versão MAT para
            # retro-compatibilidade com qualquer consumidor que ainda use os nomes
            # sem sufixo.
            # ────────────────────────────────────────────────────────────────
            'cobertura_mensal_pct': cob_mat,
            'cobertura_mensal_pct_mat': cob_mat,
            'cobertura_mensal_pct_3m':  cob_3m,
            'cobertura_mensal_pct_1m':  cob_1m,
            'cobertura_mensal_pct_parcial': cob_par,
            'medicos_sem_visita_60d': oc_mat,
            'medicos_sem_visita_60d_mat': oc_mat,
            'medicos_sem_visita_60d_3m':  oc_3m,
            'medicos_sem_visita_60d_1m':  oc_1m,
            'medicos_sem_visita_60d_parcial': oc_par,
